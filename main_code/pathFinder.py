...
def pluscourtchemin(ENZ:list, REAC:list, prod:int, n:int, imprime:bool, liste_reaction_texte:list, MOL:list, REACTIONS:list, REACPARMOL:list, bool=False) -> list:
    
    """
        ENZ:  liste des index d'enzymes
        REAC: liste des index des réactifs de base
        prod: index du produit dans la list MOL le produit
        n:    nombre max d'itération
        imprime: précise si on affiche le résultat ou pas
    ...
    # Etiquettes : a - z 
    # Il faudra gérer le cas particulier de e
    etiquettes = list(map(chr, range(97, 123)))
    # Initialisation de chaque molécule comme absente
    # List comprehension plus rapide que for
    PRESENCE = [[False, [], [], []] for _ in range(len(MOL))]
    ...
    for k in range(len(REAC)):
        # if k == 0:
        PRESENCE[REAC[k]] = [True, [(-1, 0, etiquettes[k])], [etiquettes[k]], [(-1, etiquettes[k])]]
        # if k == 1:
        #     PRESENCE[REAC[k]] = [True, [(-1, 0, 'b')], ['b'], [(-1, 'b')]]
        nbmol += 1

    # Definition du neutral (enzyme) à 'e'
        ...
        for num_molecule in range(len(PRESENCE)):
            if PRESENCE[num_molecule][0]:
                ...
                for a in REACPOT:
                    ...
                    ## on teste si tous les réactifs de la réaction en question sont présents
                    ...
                    if reactifs_presents:  ##si oui on met à jour la liste de présence
                        ...
                        elif len(REACTIONS[a][0]) == 2:  ## cas où il y a deux réactifs (cas commun)
...
def etiquette_op(etiquettes):
    # Remplacement de la fonction bin (c, d)
    result = ''.join([e for e in etiquettes if e != neutral])
    neutral = 'e' #for now

    # Eviter les for au max
    # for e in etiquettes:
    #     if e != neutral:
    #         result += e
...
def selec2ab(L1,L2):  
    """
        Cette fonction aide l'algorithme de remontée. 
        Le but est, quand le produit d'une réaction est avec l'étiquette 'ab', 
        de choisir avec quelles étiquettes on va considérer les réactifs qui ont mené à ce produit. 
        Si on a 'o' on peut choisir 'a' ou 'b' indifféremment.
    """
    l1 = selec(L1)
    l2 = selec(L2)
    if l2 == 'o':
