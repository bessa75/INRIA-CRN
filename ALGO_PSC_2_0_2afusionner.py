...
def pluscourtchemin(ENZ,REAC,prod,etqt,n):
    ...
    while (nbetape<n):
        ...
        for i in range (len(PRESENCE)):
            if PRESENCE[i][0]:
                ...
                for a in REACPOT:
                    bool=True
                    ## Test de présence des réactifs de la réaction en question
                        ...
                        if len(REACTIONS[a][0])==2: ## cas où il y a deux réactifs (cas commun)
                            ...
                            for e1 in PRESENCE[mol1][2]:
                                for e2 in PRESENCE[mol2][2]:
                                    ...
                                    for p in REACTIONS[a][1]: ##REACTIONS [a][1] correspond aux produits de la réaction a
                                        if e not in PRESENCE[p][2]:
                                            PRESENCEBIS.append((p,e))
    ...
    for meca in PRESENCE[prod][1]:
        ...
        while nbetape>0:
            ...
            PROD=[]
            for a in PRODBIS:
                if a not in PROD: ##but : ne pas reproduire les redondances de PRODBIS (s'il y en a) dans PROD
                    PROD.append(a)
            PRODBIS=[]
